...
func buildClusterSpec(c *controller.Context) (valkeyv1alpha1.ValkeyClusterSpec, error) {
	...
	// Require authentication: the default user gets a password generated by the
	// provider (ensureAuthSecret) and referenced from the auth Secret.
	spec.Users = []valkeyv1alpha1.UserAclSpec{
		{
			Name:    defaultUsername,
			Enabled: true,
			PasswordSecret: valkeyv1alpha1.PasswordSecretSpec{
				Name: authSecretName(c.Name()),
				Keys: []string{authSecretKeyPassword},
			},
			RawAcl: defaultUserACL,
		},
	}
...
// cluster's headless Service. The default user is password-protected; the
// credentials are read from the auth Secret and embedded in the connection
// URI. When transport encryption is enabled the URI uses the rediss:// scheme
// and the CA certificate is published so clients can verify the server.
func buildConnectionDetails(c *controller.Context) (controller.ConnectionDetails, error) {
	...
	password, err := readDefaultPassword(c)
	if err != nil {
	...
	cd := controller.ConnectionDetails{
		...
		Password: password,
		URI: fmt.Sprintf("%s://%s:%s@%s:%s", scheme,
			url.QueryEscape(defaultUsername), url.QueryEscape(password), host, valkeyPort),
	}
