...
CHART_DIR ?= charts/provider-valkey
...
helm-install: helm-deps ## Install the provider using Helm.
	helm install provider-valkey $(CHART_DIR) --create-namespace

.PHONY: helm-upgrade
helm-upgrade: helm-deps ## Upgrade the provider using Helm.
	helm upgrade provider-valkey $(CHART_DIR)
...
helm-template: helm-deps ## Render Helm chart templates locally (dry-run).
	helm template provider-valkey $(CHART_DIR)
